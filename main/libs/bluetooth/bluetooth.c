...
void send_data_over_bluetooth(const uint8_t *data, uint32_t len) {
    esp_spp_write(recvData->handle, len, data);
}
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_CLOSE_EVT:
        ...
        connected = false;
        ESP_LOGE(SPP_TAG, "connected = false;");
        break;
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        if (param->data_ind.len < 128) {
            ...
            for (size_t i = 0; i < recvData->len - 2 ; i++)
            {
                
                fullData[i] = recvData->data[i]; 
                fullData[i+1] = '\0'; 
            }
...
char * init_bluetooth(){
    ...
    recvData = (struct bleData *)malloc(sizeof(struct bleData));
